...
#include "paddle/fluid/framework/ir/flash_attention_fuse_pass.h"
...
inline void QKVWeightsProcess(phi::DenseTensor* wq_tensor,
  ...
  auto* bqk_data_before = bqk_tensor_before->mutable_data<T>(platform::CPUPlace());

  auto combined_w_dims = phi::make_ddim({wq_tensor->dims()[0], 3, wq_tensor->dims()[1]});
  auto combined_bias_dims = phi::make_ddim({bqk_tensor_before->dims()[1], 3, bq_tensor->dims()[0]});
  ...
  elementwise          elementwise
       |bias=[900,256]      |
         \                /
                matmul
 */
  auto tmp_bias_dims = phi::make_ddim({bqk_tensor_before->dims()[1], bq_tensor->dims()[0]});
  ...
  auto* tmp_bias_q_data = tmp_bias_q_tensor.mutable_data<T>(platform::CPUPlace());
  ...
  auto* tmp_bias_k_data = tmp_bias_k_tensor.mutable_data<T>(platform::CPUPlace());
  ...
  auto* tmp_bias_v_data = tmp_bias_v_tensor.mutable_data<T>(platform::CPUPlace());
  ...
  std::vector<T*> bias_vec = {tmp_bias_q_data, tmp_bias_k_data, tmp_bias_v_data};
  ...
  auto* tmp_combined_w_data = tmp_combined_w_tensor.mutable_data<T>(platform::CPUPlace());
  ...
  memcpy(new_combined_w_data, tmp_combined_w_data, sizeof(T) * wq_tensor->numel());
}

void FlashAttentionFusePass::ApplyImpl(ir::Graph* graph) const {
  ...
  PDNode* x = gpd.mutable_pattern()
                  ...
                  ->AsInput();
  patterns::FlashAttention pattern(gpd.mutable_pattern(), pattern_name);
  pattern(x);
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    auto* bq_tensor =
        scope->FindVar(elementwiseA1_in_y->Name())->GetMutable<phi::DenseTensor>();
    auto* bk_tensor =
        scope->FindVar(elementwiseB1_in_y->Name())->GetMutable<phi::DenseTensor>();
    auto* bv_tensor =
        scope->FindVar(elementwiseC1_in_y->Name())->GetMutable<phi::DenseTensor>();
    auto* bqk_tensor_before =
        scope->FindVar(elementwiseA0_in_y->Name())->GetMutable<phi::DenseTensor>();

    if (wq_tensor->dtype() == phi::DataType::FLOAT32) {
      QKVWeightsProcess<float>(
          wq_tensor, wk_tensor, wv_tensor, bq_tensor, bk_tensor, bv_tensor, bqk_tensor_before);
    ...
    combined_bias_desc->SetShape({bqk_tensor_before->dims()[1], 3, bq_tensor->dims()[1]});
    ...
    std::unordered_set<const Node*> marked_nodes(
        {
        elementwiseA0_op, elementwiseA0_out, matmulA0_op, matmulA0_out, elementwiseA1_op,elementwiseA1_out,
        reshapeA1_op, reshapeA1_out, transposeA1_op, transposeA1_out, scaleA1_op, scaleA1_out, matmulA1_op,
        matmulA1_out, softmaxA1_op, softmaxA1_out, matmulA2_op, matmulA2_out, transposeA2_op, transposeA2_out,
	reshapeA2_op,  elementwiseB0_op, elementwiseB0_out, matmulB0_op, matmulB0_out, elementwiseB1_op,
	elementwiseB1_out, reshapeB1_op, reshapeB1_out, transposeB1_op, transposeB1_out, matmulC0_op,matmulC0_out,
	elementwiseC1_op, elementwiseC1_out, reshapeC1_op, reshapeC1_out, transposeC1_op, transposeC1_out});
...
REGISTER_PASS(flash_attention_fuse_pass,
              paddle::framework::ir::FlashAttentionFusePass);
REGISTER_PASS_CAPABILITY(flash_attention_fuse_pass)
    .AddCombination(
