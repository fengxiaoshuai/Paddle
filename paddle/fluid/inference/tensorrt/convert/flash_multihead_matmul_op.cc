...
class FlashMultiheadMatMulOpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc& op,
    ...
    {
    // [length, 3, head_number, head_size]->[length, head_number, 3, head_size]
     auto transpose_bias_v2 = [](const float* src, float* dst, int length, int N, int H) {
          for(int l=0; l<length; l++) {
             for (int i = 0; i < 3; ++i) {
               for (int n = 0; n < N; ++n) {
                 for (int h = 0; h < H; ++h) {
                   dst[ l*3*N*H + n * 3 * H + i * H + h] = src[l*3*N*H + i * N * H + n * H + h];
                 }
               }
             }
          }
    ...
    transpose_bias_v2(bias_data_tmp.data(), bias_data, bias_length, head_number, head_size);
   ...
   nvinfer1::Weights bias{nvinfer1::DataType::kFLOAT,
    ...
    nvinfer1::MatrixOperation matrix_operation_X = nvinfer1::MatrixOperation::kNONE;
    nvinfer1::MatrixOperation matrix_operation_Y = nvinfer1::MatrixOperation::kTRANSPOSE;
    merge_matmul_layer = TRT_ENGINE_ADD_LAYER(engine_, MatrixMultiply, *input, matrix_operation_X, *weight_tensor, matrix_operation_Y);
    merge_matmul_layer->setName( ("flash_attention_matrix_multiply(Output: " + output_name + ")").c_str() );

    // add ElementWiseLayer layer
    nvinfer1::ElementWiseOperation elementwise_operation =  nvinfer1::ElementWiseOperation::kSUM;
    merge_element_layer = TRT_ENGINE_ADD_LAYER(engine_, ElementWise, *merge_matmul_layer->getOutput(0), *bias_tensor, elementwise_operation);
    merge_element_layer->setName(("flash_attention_elementwise(Output: " + output_name + ")").c_str());

    }
    // add shuffle

    auto* reshape_after_fc_layer =
        TRT_ENGINE_ADD_LAYER(engine_, Shuffle, *merge_element_layer->getOutput(0));
